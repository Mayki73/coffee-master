...
export const getStaticPaths = (async () => {
  return {
    paths: Mulfunctions["mulfunctions"].map((mulfunction) => ({
      params: { mulfunction: mulfunction.link },
    })),
...
export const getStaticProps = (async ({ params }) => {
  return {
    props: {
      mulfunction: Mulfunctions["mulfunctions"].find(
        (mulfunction) => mulfunction.link === params?.mulfunction
      ),
    },
  };
}) satisfies GetStaticProps;

const MulfunctionTemplate: React.FC<any> = ({ mulfunction }) => {
  ...
  const { title, description, prices, seo } = mulfunction;
  const { advantages } = Mulfunctions;
