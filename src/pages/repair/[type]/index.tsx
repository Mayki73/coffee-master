...
export const getStaticPaths = (async () => {
  return {
    paths: Brands["machines"].map((machine) => ({
      params: { type: machine.type },
    })),
...
export const getStaticProps = (async ({ params }) => {
  return {
    props: {
      type: Brands["machines"].find((machine) => machine.type === params?.type),
    },
  };
}) satisfies GetStaticProps;

const MachineTypeTemplate: React.FC<any> = ({ type }) => {
    ...
    <>
      ...
      >
        ...
        <main className="max-w-screen-2xl mx-5 md:mx-auto my-20 font-[Comfortaa] text-base md:text-lg leading-8 tracking-wider text-gray-600 font-light space-y-10">
          ...
          <ScrollAnimate>
            <p>
              Если вы не нашли свой бренд в списке или не знаете, как его
              определить – <button className="font-bold">напишите</button> или{" "}
              <button className="font-bold">позвоните</button> нам. Мы
              подскажем, как это сделать и сможем починить вашу кофемашину!
            </p>
          </ScrollAnimate>
                  ...
                  <div className="p-5 border-2 border-black space-y-5">
                    ...
                    {advantage.descriptionType === "text" ? (
                      ...
                      <ul className="list-disc list-inside space-y-4">
                        {advantage.description.map(
                          (descriptionListItem: any) => (
                            <li key={descriptionListItem}>
                              {descriptionListItem}
                            </li>
                          )
                        )}
